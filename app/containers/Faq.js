...
class Faq extends Component {
    ...
    componentDidMount(){
        ...
        this.props.getContent().then((resp)=>{
            ...
            var content = _.filter(resp.response,{'contentType':'FAQ'});
    ...
    _renderItem(){
        ...
        for(let i=0; i<this.state.dataSource.length;i++){
            html = html + '<h3 class="title">'+this.state.dataSource[i].title+'</h3>';
            html = html + '<p class="desc">'+this.state.dataSource[i].description+'</p>'
        }

        faq.push(<HTMLView value={html} stylesheet={htmlStyles}></HTMLView>)
